...
- Tracks per-user hint escalation (level 1 â†’ 2 â†’ 3) across submissions.
...
project root/
â”œâ”€â”€ backend/
â”‚   â”œâ”€â”€ app.py              # Flask application factory + global error handlers
â”‚   â”œâ”€â”€ routes.py           # API Blueprint (all route handlers)
â”‚   â”œâ”€â”€ java_engine.py      # Secure Java compile-and-execute engine
â”‚   â”œâ”€â”€ db.py               # MongoDB persistence layer (submissions collection)
â”‚   â”œâ”€â”€ response.py         # Unified JSON envelope builders: ok() / fail()
â”‚   â”œâ”€â”€ llm.py              # LLM hint generation via LM Studio (fully implemented)
â”‚   â”œâ”€â”€ hint_manager.py     # Progressive hint escalation tracker (hint_state collection)
â”‚   â””â”€â”€ README.md           # This file
â”‚
â”œâ”€â”€ test_flask_backend.py   # 49 unit tests for routes + app
â”œâ”€â”€ test_llm.py             # 65 unit tests for llm.py
â”œâ”€â”€ test_hint_manager.py    # 41 unit tests for hint_manager.py
â”œâ”€â”€ live_llm_test.py        # Live 5-test integration test against LM Studio
â”‚
â”œâ”€â”€ master_prompt.txt       # Project specification / AI context document
â”œâ”€â”€ .gitignore
...
HTTP response â€” including Flask-generated error pages â€” uses the same JSON
...
**validate â†’ execute â†’ hint â†’ log â†’ respond**. No pymongo, no subprocess, no
OpenAI imports live here.

Current routes:
- `GET  /api/health`      â€” liveness probe
- `POST /api/submit-code` â€” main code submission endpoint
...
`log_submission(user_id, code, result)`. Absorbs `PyMongoError` â€” a DB failure
...
### `response.py`
Zero-dependency envelope builder. No Flask routes, no DB imports â€” safe to
import anywhere. Provides:
- `ErrorCode` â€” string constants: `MISSING_FIELD`, `INVALID_INPUT`,
  `EXECUTION_FAILED`, `INTERNAL_ERROR`, `NOT_FOUND`, `METHOD_NOT_ALLOWED`, `TIMEOUT`
- `ok(data, http_status=200)` â†’ `{"success": True, "data": data, "error": None}`
- `fail(message, code, details, http_status=400)` â†’ `{"success": False, "data": None, "error": {...}}`
...
- Uses `requests.post()` directly â€” no `openai` SDK dependency.
- `_build_system_prompt()` â€” defines teaching persona, enforces no-full-solution
  policy, bans code snippets > 2 lines, demands JSON-only output with 6 keys.
- `_build_user_prompt(code, execution_result)` â€” embeds code + status +
  error_message + line_number + exception_type.
- `_call_llm(system_prompt, user_prompt)` â€” POST to `LLM_API_URL`,
  temperature=0.3, max_tokens=1200, raises `HTTPError` on 4xx/5xx.
- `_parse_llm_response(raw_text)` â€” tries direct JSON parse â†’ extracts `{...}`
  block â†’ safe fallback dict.
- `_filter_by_hint_level(hints, hint_level)` â€” always returns
  `problem_summary + why + learning_tip`; adds `hint_1` (level â‰¥ 1),
  `hint_2` (level â‰¥ 2), `hint_3` (level â‰¥ 3).
- `generate_hints(code, execution_result, hint_level=1)` â€” **main public API**,
  returns structured dict or `{"status": "LLMError", "message": "..."}`.
- `generate_hint(code, result)` â€” **backward-compatible shim** for `routes.py`;
...
requested for a specific submission and controls which hint level (1 â†’ 2 â†’ 3)
...
Escalation rules:
1. First time user encounters error â†’ create record, return level 1
2. Same code + same error, called again â†’ increment level
3. Maximum level = 3 (capped, never exceeds `MAX_HINT_LEVEL = 3`)
4. Error type changes â†’ reset to level 1
5. `status == "Success"` â†’ call `reset_hint_level()` from route
...
| pip packages | â€” | `flask`, `pymongo`, `requests` |

Install Python dependencies:

```powershell
# From the project root â€” activate venv first
...
```powershell
# Correct â€” uses the venv Python with all installed packages
...
### Production (Linux/Mac â€” Gunicorn)
...
Every response from this API â€” including framework 404/405 errors â€” uses the
...
1. Builds a system prompt defining the teaching persona (no full solutions,
   code snippets â‰¤ 2 lines, JSON-only output with 6 required keys).
...
```python
{
    ...
    "hint_1":          "Gentlest hint â€” points to the area of the problem.",
    "hint_2":          "Stronger hint â€” describes what needs to change.",  # level >= 2
    "hint_3":          "Most direct hint â€” may include up to 2 lines of code.",  # level >= 3
...
code submission and controls which hint level (1 â†’ 2 â†’ 3) to provide. It is
**not yet wired into `routes.py`** â€” see [Wiring guide](#wiring-hint_manager-into-routespy).

### Escalation sequence

```
User submits buggy code (new)  â†’ update_hint_level() â†’ returns 1
User clicks "get another hint" â†’ update_hint_level() â†’ returns 2
User clicks "get another hint" â†’ update_hint_level() â†’ returns 3
User clicks "get another hint" â†’ update_hint_level() â†’ returns 3  (capped)
User fixes code, resubmits     â†’ reset_hint_level()  â†’ level reset to 1
User introduces a new bug      â†’ update_hint_level() â†’ error changed â†’ returns 1
```
...
| `hint_level` | int | Current level (1â€“3) |
...
All tests use mocks â€” no running Flask server, MongoDB, Java, or LLM required.
...
- `TestHealth` (6) â€” GET /api/health envelope and methods
- `TestInputValidation` (10) â€” missing/blank/invalid fields
- `TestExecutionOutcomes` (13) â€” all four statuses, hint propagation
- `TestHttpStatusCodes` (4) â€” 200 / 408 mapping
- `TestMongoLogging` (6) â€” log_submission calls and fault tolerance
- `TestGlobalErrorHandlers` (3) â€” 404 / 405 as JSON
- `TestResponseEnvelope` (7) â€” envelope shape on success and failure

**`test_llm.py`** (65 tests)
- `TestBuildSystemPrompt` (9) â€” teaching policy rules in prompt
- `TestBuildUserPrompt` (9) â€” code + execution context embedding
- `TestParseLlmResponse` (7) â€” JSON parse, markdown fence extraction, fallback
- `TestFilterByHintLevel` (11) â€” progressive unlock logic
- `TestGenerateHintsDisabled` (4) â€” LLM_ENABLED=false path
- `TestGenerateHintsHappyPath` (9) â€” successful LLM call + filtering
- `TestGenerateHintsErrors` (7) â€” ConnectionError, Timeout, HTTPError, etc.
- `TestGenerateHintShim` (9) â€” backward-compat shim string formatting

**`test_hint_manager.py`** (41 tests)
- `TestHashCode` (4) â€” SHA-256 correctness
- `TestGetCurrentHintLevelNew` (4) â€” no-record path â†’ level 1
- `TestGetCurrentHintLevelKnown` (5) â€” stored doc scenarios
- `TestUpdateHintLevelNew` (5) â€” first-submission insert
- `TestUpdateHintLevelSameError` (6) â€” increment + cap at MAX
- `TestUpdateHintLevelErrorChange` (4) â€” error type reset
- `TestResetHintLevel` (6) â€” resolved flag + counter reset
- `TestDatabaseErrors` (7) â€” PyMongoError fault tolerance
...
**Step 1 â€” Add imports at the top of `routes.py`:**

```python
from hint_manager import reset_hint_level, update_hint_level
```

**Step 2 â€” Replace the LLM hint block inside `submit_code()`.** Find this
...
hint: Optional[str] = None
if result["status"] == "Success":
    # Issue resolved â€” reset escalation counter so next bug starts at level 1
    try:
...
**Step 3 â€” Optionally expose `hint_level` in the response** so the frontend
...
- First request â†’ `update_hint_level` called once, returns 1
- Second request same error â†’ `update_hint_level` returns 2
- `Success` result â†’ `reset_hint_level` called, `update_hint_level` not called
...
5. **All `PyMongoError` exceptions must be caught** in `db.py` and
   `hint_manager.py` â€” a DB failure must never return an error to the client.
6. **All LLM errors must be non-fatal** in `llm.py` â€” the API still returns
...
8. The `hint` field in the API response is always a **plain string or `null`**
   (never a raw dict) â€” the `generate_hint()` shim handles collapse.
9. `LLM_ENABLED` defaults to `false` â€” enabling requires an explicit env var.
...
2. **Expose `hint_level` in the API response** â€” add `data.hint_level` so the
   frontend can display "Hint 1 of 3", "Hint 2 of 3", etc.
3. **Build the frontend** â€” React or plain HTML/JS that calls `/api/submit-code`
   and renders execution results + hints with a "Get another hint" button.
4. **Add `GET /api/hint-status/{user_id}`** â€” lets the frontend query current
   hint level without requiring a new code submission.
5. **Add authentication** â€” replace bare `user_id` string with JWT or session
   tokens so hint state cannot be spoofed.
6. **Add rate limiting** â€” prevent hint-farming via rapid identical submissions.
